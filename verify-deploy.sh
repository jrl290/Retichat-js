#!/usr/bin/env bash
#
# verify-deploy.sh — does the app retichat.com is serving match a git ref?
#
#   ./verify-deploy.sh              # compare the live app against HEAD
...
REF="${1:-HEAD}"
BASE_URL="${RETICHAT_WEB_URL:-https://retichat.com}"
...
if [[ $DRIFT -gt 0 ]]; then
  ...
  echo "${DIM}  curl -s ${BASE_URL}/${DRIFTED_FILES[0]} | diff - <(git show ${REF}:${DRIFTED_FILES[0]})${NC}"
  echo "${DIM}To make the node match: ./deploy.sh ${REF}${NC}"
fi
exit 1
