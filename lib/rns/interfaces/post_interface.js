...
class PostInterface extends Interface {
    ...
    async connect() {
        ...
        // Tell the node when this page goes away, so it stops holding our
        // registration (and the local destinations behind it) until its
        // stale timeout. pagehide is the one unload event browsers deliver
        // reliably; the beacon is best effort and the node's timeout stays
        // as the backstop for crashes and lost networks.
        if (!this._goodbyeHook && typeof window !== 'undefined' && window.addEventListener) {
            this._goodbyeHook = () => this.goodbye();
            window.addEventListener('pagehide', this._goodbyeHook);
        }
    ...
    async _flushIfIdle() {
        if (!this.isRegistered || !this._running) return;
    ...
    async _runExchange() {
        if (this._exchangeInFlight) return;
