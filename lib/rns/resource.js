...
class Resource extends EventEmitter {
    ...
    constructor(link) {
        ...
        // response (ResourceAdvertisement.__init__, :1305-1315). RNS/Link.py
        // request() sends a request larger than the MDU this way and
        // handle_request() its response; the receiver dispatches by these
        // flags before any resource strategy applies (Link.py:1036-1066).
        this.requestId = null;
    ...
    packAdvertisement(segment) {
        ...
        // encrypted (bit 0); never compressed (1), split (2) or with
        // metadata (5); request (3) and response (4) when carrying one.
        const flags = 0x01 | ((this.isRequest ? 1 : 0) << 3) | ((this.isResponse ? 1 : 0) << 4);
    ...
    applyAdvertisement(advertisement) {
        ...
        this.isRequest = ((flags >> 3) & 0x01) === 0x01;
        this.isResponse = ((flags >> 4) & 0x01) === 0x01;
