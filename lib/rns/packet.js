...
class Packet {
    ...
    static KEEPALIVE      = 0xFA; // Packet is a keepalive packet (unencrypted 1 byte: 0xFF ping / 0xFE pong)
    ...
    pack() {
        ...
        if(this.context === Packet.LRPROOF) {
            ...
            if(this.packetType === Packet.ANNOUNCE){
                ...
                ciphertext = this.data;
            } else if(this.packetType === Packet.PROOF && this.context === Packet.RESOURCE_PRF){
                // resource proofs are not encrypted
                ciphertext = this.data;
            } else if(this.packetType === Packet.LINKREQUEST){
                // link request packets are not encrypted
                ciphertext = this.data;
            } else if(this.context === Packet.RESOURCE){
                // a resource encrypts its payload as a whole before splitting
                // it into parts, so parts are not encrypted again here
                ciphertext = this.data;
            } else if(this.context === Packet.KEEPALIVE){
                // KEEPALIVE travels UNENCRYPTED: a single byte, 0xFF ping /
                // 0xFE pong. Reticulum-rust/src/link.rs handles it BEFORE the
                // decrypt step and comments why — "self.decrypt() will fail on
                // a 1-byte input and we'll silently return Ok(()) without ever
                // sending a reply", which stops the initiator's last_inbound
                // ever refreshing and tears the link down at stale_time.
                // Note this is ONLY KEEPALIVE: LINKIDENTIFY (0xFB) and LRRTT
                // (0xFE) sit above it numerically but ARE encrypted.
                ciphertext = this.data;
            } else {
    ...
    getHashablePart() {
        ...
        // Lower nibble of flags byte + everything after flags+hops.
        // Mirrors Python Packet.get_hashable_part().
