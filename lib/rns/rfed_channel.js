...
async function channelComputeStamp(wirePayload, cost) {
    try {
        ...
        // Step 2: the LXMF workblock — 256 bytes of HKDF-SHA256 per round,
        // concatenated, salted with sha256(material || msgpack_uint(n)).
        //
        // This used to iterate sha256 over a single 32-byte value, matching a
        // Reticulum-rust LXStamper that had itself diverged from LXMF's Python
        // implementation. Both sides now follow the reference: the algorithm is
        // shared with the wider Reticulum network, so we do not get to pick our
        // own. At 16 rounds the workblock is 4 KB, so parity costs nothing here.
        const msgpackUint = (n) => {
        ...
        for (let n = 0; n < STAMP_EXPAND_ROUNDS; n++) {
            const salt = sha256(Buffer.concat([transientId, msgpackUint(n)]));
            parts.push(Buffer.from(hkdf(sha256, transientId, salt, '', 256)));
        }
        const workblock = Buffer.concat(parts);

        // Step 3: mine a random 32-byte stamp where sha256(workblock || stamp)
        // has >= cost leading zero bits. Random, as the reference does — the
        // old nonce-derived stamp made every client produce the same bytes.
        const target = cost;
        const stamp = Buffer.alloc(32);
        for (let attempt = 1; attempt <= 5_000_000; attempt++) {
            crypto.getRandomValues(stamp);
            const hash = sha256(Buffer.concat([workblock, stamp]));
            let leadingZeros = 0;
            ...
            if (leadingZeros >= target) {
                console.log(`[channel] 🔨 Stamp found after ${attempt} attempts (${leadingZeros} leading zero bits, target ${target})`);
                return Buffer.from(stamp);
            }
