...
import LXMF, { GROUP_FIELDS } from "./lxmf/lxmf.js?v=20260726-keys2";
import LXMessage from "./lxmf/lxmf_message.js?v=20260726-keys2";
...
} from "./rfed_channel.js?v=20260816-stamp-parity";
...
class Reticulum extends EventEmitter {
    ...
    registerDestination(identity, direction, type, appName, ...aspects) {
        ...
        // `this.destinations` is the INBOUND DISPATCH TABLE, not a list of every
        // destination we know. Only IN destinations belong in it, and a hash may
        // appear at most once. Reference: RNS/Transport.py register_destination(),
        // which registers only `direction == IN` and raises on a duplicate hash.
        //
        // UNFORGIVABLE REGRESSION IF BROKEN AGAIN — see destination_registry.test.mjs.
        // OUT destinations are built from public-key-only identities. If one reaches
        // this table it matches inbound packets sent to our OWN hash (which happens
        // the moment you message yourself, or message an address you have also
        // adopted, e.g. distro), then `onPacket` overwrites `packet.destination`
        // with the key-less copy. The two symptoms are always these:
        //   "Failed to parse packet: scalar must be hex string or Uint8Array"  (decrypt)
        //   "private key must be hex string or Uint8Array"                     (prove/sign)
        if(direction !== Destination.IN){
            return destination;
        }

        // Return the existing registration rather than adding a second one. A new
        // object would carry none of the listeners already attached by LXMRouter,
        // so inbound messages would silently stop being delivered.
        const registered = this.destinations.find((existing) => existing.hash.equals(destination.hash));
        if(registered){
            return registered;
        }
    ...
    onPacketReceived(packet, receivingInterface) {
        ...
        } else if(packet.packetType === Packet.PROOF && packet.context === Packet.RESOURCE_PRF) {

            // A resource proof is addressed to the link, and its payload is the
            // resource hash plus the proof — not a packet hash, so it must not
            // fall through to the delivery-proof handler below.
            for(const link of this.links){
                if(link.hash.equals(packet.destinationHash)){
                    link.onResourceProof(packet);
                }
            }
