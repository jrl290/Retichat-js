...
class Link extends EventEmitter {
    ...
    onPacket(packet) {
        ...
        // an advertisement offers a payload too large for a single packet
            ...
            // response is accepted before, and regardless of, the resource
            // strategy — it is a link request that did not fit one packet,
            // and the strategy only governs bare data. Until 2026-09-22 every
            // advertisement went through the strategy and concluded as bare
            // data, so an rfed.link push larger than the MDU was assembled,
            // handed to the channel ingest under the wrong path, and never
            // answered — it fell to the deferred queue and /channel/pull.
            const get = (key) => (advertisement instanceof Map ? advertisement.get(key) : advertisement?.[key]);
            const flags = Number(get("f") ?? 0);
            const carriesRequestOrResponse = !!get("q") && (((flags >> 3) & 0x01) === 0x01 || ((flags >> 4) & 0x01) === 0x01);
            if(!carriesRequestOrResponse && this.resourceStrategy === Link.ACCEPT_NONE){
                return;
            }
            const resource = Resource.accept(this, advertisement);
            if(resource){
                resource.once("concluded", () => this._dispatchConcludedResource(resource));
            }
        }
    ...
    _dispatchConcludedResource(resource) {
        ...
        if(resource.isRequest){
            ...
            this.emit("response", { requestId: parsed[0], data: parsed[1], resource });
        } else {
     ...
     * Either way the response arrives on the "response" event.
     */
    _sendRequestPayload(requestPayload) {
        ...
        Resource.send(this, requestPayload, { requestId, isRequest: true }).catch((e) => {
            console.warn(`[link] request resource ${requestId.toString("hex").slice(0,12)} failed: ${e.message}`);
        });
        return requestId;
    }
    ...
    sendRequest(path, data) {
        ...
        return this._sendRequestPayload(requestPayload);
    }
